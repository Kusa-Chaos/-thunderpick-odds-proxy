...
// manual-live-test-trigger: 2026-09-24 player-prop verification
...
function classifyMarket(m={}){
 ...
 // Half/segment winners are distinct three-way/two-way contracts and must
 // never fall through to plain Map Winner or Match Winner.
 if(/\b(?:1st|first|2nd|second)\s+half\b/.test(text)&&/\b(?:winner|1x2|moneyline)\b/.test(text))return 'map_half_winner';
 if(!/round/.test(text)&&/(handicap|spread)/.test(text))return 'spreads';
 if(!/round/.test(text)&&/total/.test(text))return 'totals';
 // Plain map winner only. Explicit half/segment/1X2 labels are excluded.
 if(/map/.test(text)&&/\bwinner\b/.test(text)&&!/round/.test(text)&&!/\b(?:1st|first|2nd|second)\s+half\b/.test(text)&&!/\b1x2\b/.test(text))return 'map_winner';
 if(/\b(winner|moneyline)\b/.test(text)&&!(/map|round|half/.test(text))&&!/\b1x2\b/.test(text))return 'h2h';
 return null;
}
...
const oneWayPlayerPropScreensOut=[];const playerPropInventory=[];
for(const sport of SPORTS){
 ...
 for(const e of tpEvents(sport)){
  ...
  for(const d of markets){
   if(d.key==='player_prop') playerPropInventory.push({tier:'INFORMATIONAL',sport,match:e.name||`${e?.teams?.home?.name||e?.market?.home?.name||''} vs ${e?.teams?.away?.name||e?.market?.away?.name||''}`,eventId:e.id,startTime:e.startTime,player:d.prop?.player||null,stat:d.prop?.stat||null,market:d.label,line:d.selections?.[0]?.point??null,scope:d.scope,thunderpick:{over:d.selections?.[0]?.odds??null,under:d.selections?.[1]?.odds??null},independentSources:0,blocker:'Thunderpick player prop captured; no exact outside contract currently available for fair-value verification'});
